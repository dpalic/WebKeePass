...
import webBoltOns.client.components.CTabTableContainer;
import webBoltOns.client.components.CTableColumn;
 

public class TableColumnSorter extends TableSorterMap {

	private static final long serialVersionUID = -3993970597137998867L;
	int indexes[];
	Vector sortingColumns = new Vector();
	boolean ascending = true;
	int compares;
	protected CTabTableContainer tabs;
	protected CTableColumn[] tableColumns;
	protected AppletConnector appletConnector;
	public TableColumnSorter() {
		indexes = new int[0];  
	}

	public TableColumnSorter(TableModel model) {
		setModel(model);
	}

	public TableColumnSorter(TableModel model, CTabTableContainer tabs,
			CTableColumn[] tableColumns, AppletConnector appletConnector) {
		this.tabs = tabs;
		...
		for (int i = 0; i < getRowCount(); i++) {
			for (int j = i + 1; j < getRowCount(); j++) {
				if (compare(indexes[i], indexes[j]) == 1) {
					swap(i, j);
					if(tabs != null)
						tabs.fireTableSorting(this, i, j);
				}
			}
		}
	}
	...
	public void addMouseListenerToHeaderInTable(JTable table) {
		final TableColumnSorter sorter = this;
		...
		MouseAdapter listMouseListener = new MouseAdapter() {
			public void mouseClicked(MouseEvent e)  {
				...
				TableColumn vCol = columnModel.getColumn(viewColumn);				 
				if (column != -1) {
					th.setCursor(new Cursor(Cursor.WAIT_CURSOR));
					CTableColumn.CTableHeaderRenderer hd = 
						(CTableColumn.CTableHeaderRenderer ) vCol.getHeaderRenderer();					
 					boolean ascending = true;
