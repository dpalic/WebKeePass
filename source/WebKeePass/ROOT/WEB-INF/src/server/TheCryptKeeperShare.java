...
public class TheCryptKeeperShare {

	public TheCryptKeeperShare() {
	}
	...
	public DataSet getCryptKeeper(DataSet dataSet, DataAccess dataAccss) {
		try {
			if (!dataAccss.executeQuery( "Select * From wkpGroups Where KeeperID = "
							+ dataSet.getIntegerField("KeeperID")
							+ " And j1 = '[Share-Group/]' ", "wkpGroups", dataSet));
	...
	private DataSet insertCryptKeeper(DataSet dataSet, DataAccess dataAccss) {
		try {
			dataSet.putStringField("CreateDate", new SimpleDateFormat("yyyyMMdd").format(new Date()) );
			dataSet.putStringField("LastUpdate", new SimpleDateFormat("yyyyMMdd").format(new Date()) );
			dataSet.putStringField("j1", "[Share-Group/]");
		...
		try {
			if (!dataAccss.executeQuery(
					"Select * From wkpGroups Where KeeperID > "
							+ dataSet.getIntegerField("KeeperID")
							+ " And j1 = '[Share-Group/]' Order By KeeperID ", "wkpGroups", dataSet))
		...
		try {
			if (!dataAccss
					.executeQuery("Select * From wkpGroups Where KeeperID < "
							+ dataSet.getIntegerField("KeeperID")
							+ " And j1 = '[Share-Group/]' Order By KeeperID Desc ", "wkpGroups", dataSet))
	...
	public DataSet getwkpGroupsList(DataSet dataSet, DataAccess dataAccss) {
		...
		String keyField1 =   dataSet.getStringField("@GroupID");
		String[] tblCols =   dataSet.getTableColumnFields("Table1");
		ResultSet resultSet;
		Statement sqlStatement = dataAccss.execConnectReadOnly();

		String sql = "Select " + DataAccess.buildArgumentList(tblCols)
				+ "  From wkpGroups Where j1 = '[Share-Group/]' ";
