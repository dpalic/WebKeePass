...
import webBoltOns.server.servletUtil.NativeExecutable;
 ...
 public class ServletConnector extends DaemonRemoteServlet implements
	...
	private String jRivetVersion = "    --jRivet Framework - version: v 3.060506 2006/05/06   08:15:39 jrivet.com-- ";
	@Override
	public void destroy() {
		super.destroy();
	}
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			if (action.equals(MenuItem.GET_ICONS))
				...
				dataSet = getUserEnvironment(dataSet, req);
	...
	public DataSet getMenu(DataSet dataSet) {
		...
		try {
			String script;
			if (dataSet.getSecurityManagerStatus())
				script = dataAccess.getMenuPath() + dataSet.getStringField("[MENU_SCRIPT/]") + ".xml";
			else
				script = dataAccess.getMenuPath() + dataAccess.getStandardMenu() + ".xml";
	...
	public DataSet getScript(DataSet dataSet) {
		...
		String script = dataAccess.getScriptPath()
				+ dataSet.getStringField("[SCREEN_SCRIPT/]") + ".xml";
		try {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		String configFile = "ConfigFile1.xml";

		if (configHome != null)
			dataAccess = new DataAccess(configHome + configFile, this);
		else
			dataAccess = new DataAccess("/java/" + configFile, this);
		...
		if (dataAccess.initDataAccess()) {
			...
			dataAccess.logMessage(".....Script-Path : " + scriptFilePath);
			dataAccess.logMessage(".....Image-Path : " + imageFilePath);
	 ...
	 *  
	 */
	private DataSet getUserEnvironment(DataSet dataSet, HttpServletRequest req) {
		if (dataAccess.isSecurityManagerON()) {
