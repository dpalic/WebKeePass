...
import webBoltOns.server.reportWriter.ReportHandler;
...
public class DevelopmentTools {
    ...
    public DataSet getScreenScriptDetails(DataSet script, DataAccess dataAccess) {
 	...
 	public DataSet getReportWriterDetails(DataSet script, DataAccess dataAccess) {
		String scriptFileName = script.getStringField("ScriptID"),
			docPath;
		try {
			docPath = dataAccess.getScriptPath() + scriptFileName + ".xml";
			...
			sp.parse(new InputSource(reader), handler);
			DataSet scriptTable = handler.getReportTable();		
			if(handler.isValidXMLType("[REPORT_SCRIPT/]")) {
				script.putStringField("SqlQry", scriptTable.getStringField(ReportColumn.SQL_QUERY));
				script.putStringField("RptDesc",scriptTable.getStringField(ReportColumn.DESCRIPTION));
				script.put("REditor", scriptTable);
			} else {
				script.addMessage("Invalid Report Document", "30", null, null);
				script.put("REditor", null);
			}
		} catch (Exception e) {
			script.put("SEditor", null);
			script.addMessage("Script Not Found", "30", null, null);
	...
	public DataSet saveReportWriterDetails(DataSet script, DataAccess dataAccess) {
	try {
		XmlDocument xml = new XmlDocument();
		Element root = xml.createElement("SCRIPT");
		String scriptname = script.getStringField("ScriptID");
		...
		String today = (new SimpleDateFormat("dd-MMM-yyyy").format(new Date()));
       	...
       	xml.appendChild(xml.createComment(
       			       "\t **Script Builder 1.0 *  ")); 
    	xml.appendChild(xml.createComment(
    	 		       "\t \t Report Script Name :  " + scriptname+  " " ));
       	xml.appendChild(xml.createComment(
       			       "\t \t Description :  " + script.getStringField("RptDesc") +  " " ));
       	xml.appendChild(xml.createComment(
			       "\t \t Report Qry :  \n\n " + script.getStringField("SqlQry") +   "\n\n " ));
       	xml.appendChild(xml.createComment(
    			       "\t \t File Path :  " + scriptpath +  " "));
    	xml.appendChild(xml.createComment(
    			       "\t \t Last Update :  " + today +  " "));
    	xml.appendChild(xml.createComment(
       	...
       	setRootAttribute(root, "SCRIPT_NAME", scriptname);
        setRootAttribute(root, ReportColumn.DOCUMENT_TYPE , "Report");
       	setRootAttribute(root, ReportColumn.DESCRIPTION , script.getStringField("RptDesc"));
        
		Element sqlQry = xml.createElement("SqlQuery"); 
        sqlQry.appendChild(xml.createTextNode(script.getStringField("SqlQry")));
        root.appendChild(sqlQry);
     
		Enumeration  elements = script.getTableVector("[Path/]").elements();
		while (elements.hasMoreElements()) {
			Element columnComponent = xml.createElement("Column");   
			ReportColumn rc  = (ReportColumn)elements.nextElement();
			if(rc != null ) {
				setReportAttribute(columnComponent, ReportColumn.FIELDNAME, rc.getFeildName());
				setReportAttribute(columnComponent, ReportColumn.DESCRIPTION, rc.getDescription());
				setReportAttribute(columnComponent, ReportColumn.DATATYPE, rc.getDataType());
				setReportAttribute(columnComponent, ReportColumn.LENGTH, Integer.toString(rc.getLength()));
				setReportAttribute(columnComponent, ReportColumn.DECIMALS, Integer.toString(rc.getDecimals()));
				setReportAttribute(columnComponent, ReportColumn.ALIGNMENT, rc.getAlignment());
				setReportAttribute(columnComponent, ReportColumn.LEVELBREAK, Integer.toString(rc.getLevelBreak()));
	
				if(rc.isSubTotaled())
					setReportAttribute(columnComponent, ReportColumn.SUB_TOTAL, "Y");
				else
					setReportAttribute(columnComponent, ReportColumn.SUB_TOTAL, "N");

				if(rc.isSubMaximum())
					setReportAttribute(columnComponent, ReportColumn.SUB_MAXIMUM, "Y");
				else
					setReportAttribute(columnComponent, ReportColumn.SUB_MAXIMUM, "N");

				if(rc.isSubMinumum())
					setReportAttribute(columnComponent, ReportColumn.SUB_MINIMUM, "Y");
				else
					setReportAttribute(columnComponent, ReportColumn.SUB_MINIMUM, "N");

				if(rc.isSubAveraged())
					setReportAttribute(columnComponent, ReportColumn.SUB_AVERAGE, "Y");
				else
					setReportAttribute(columnComponent, ReportColumn.SUB_AVERAGE, "N");

				if(rc.isSubCounted())
					setReportAttribute(columnComponent, ReportColumn.SUB_COUNT, "Y");
				else
					setReportAttribute(columnComponent, ReportColumn.SUB_COUNT, "N");

				root.appendChild(columnComponent);
			}
		}  
