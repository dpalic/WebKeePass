...
public class MenuFrame extends JPanel implements ComManager, ClipboardOwner,  MouseListener, KeyListener, ActionListener {
	...
	private final String adminScpMth[] = { null, null, null, null, null,
			"getMenuLink", null, null, null, null };

	private final String adminScpOp[] = { "UserAdmin110",
			"UserAdmin130", "UserAdmin100", "UserAdmin120", "UserAdmin150",
			"UserAdmin160", "UserAdmin190", "UserAdmin180", "UserAdmin210", 
			"UserAdmin220" };

	private final String adminTxtOp[] = { "Edit User", "User Profiles",
			"Edit Groups", "Group Profiles", "Edit Script", "Edit Menu",
			"Script Profiles", "Create Server Base", "Edit Reports",
			"Edit Company" };
	...
	private JPanel buildAdminTools(DefaultMutableTreeNode adminTree, JComboBox combomode) {
		...
		titledBorder = new TitledBorder(border, "Server Information");
		...
		border = BorderFactory.createLineBorder(new Color(0, 0, 150), 1);
		titledBorder = new TitledBorder(border, "Server Logs");
		titledBorder.setTitleFont(cntr.standardFont);
		titledBorder.setTitleColor(new Color(0, 0, 150));
		...
		JButton logButton = new JButton("View Server Log",cntr.quickLinkIcon);
		...
		border = BorderFactory.createLineBorder(new Color(0, 0, 150), 1);
		titledBorder = new TitledBorder(border, "User Security / Edit XML");
		titledBorder.setTitleFont(cntr.standardFont);
		titledBorder.setTitleColor(new Color(0, 0, 150));
		...
		for (int b = 0; b < adminScpOp.length; b++) {
			final int x = b;
			adminButton[x] = new JButton(adminTxtOp[x], cntr.quickLinkIcon);
			adminButton[x].setBorder(null);
			DefaultMutableTreeNode adminNode = new DefaultMutableTreeNode(new JLabel(adminTxtOp[x]));
		...
		adminPanel.add(adminButton[7], new GridFlowLayoutParameter(false, 1));
		adminPanel.add(adminButton[8], new GridFlowLayoutParameter(false, 2));
	...
	private void buildMainMenu(DataSet menu) {
		...
		infoT = cntr.buildFancyButton("", "logo.gif","Admin/User Tools", ' ');
		...
		slpter = cntr.buildFancyButton("   ", "left.gif", "Set Split", ' ');
...
/*  */
	...
	private void loadAdminScript(String scriptToRun, String initMethod,
		...
		if (initMethod != null) {
			adminWindow.getDataSet().putStringField("MenuXML",
					cntr.menuScript);
		...
		if (e.getSource().equals(menuTree) && e.getClickCount() == 2) {
			...
			if (node != null && e.getClickCount() == 2) {
				...
				for (int m = 0; m < adminTxtOp.length; m++) {
					if (menuitem.equals(adminTxtOp[m])) {
